...
public class Frontend  {
    ...
    public Frontend(String[] input) {
        this.backend = new BackendDummy(input);
        this.topThreeMovies =  backend.getThreeMovies(10);
        this.genres = (String[]) backend.getGenres().toArray();

    }
    public Frontend(Reader r) {
        this.backend = new BackendDummy(r);
    }
    public void run(){
        sortTopThree();
        boolean exit = false;
    ...
    public boolean ratingsMenu() {
        ...
        for(Character c : s.toCharArray()) {
            if(c.compareTo(' ')!= 0){
                responses.add(Integer.valueOf(c));
            }
        }
