...
public class TestFrontend {
    ...
    public void runTests() {
        ...
        System.out.print("Test 'r' loads the rating screen (WARNING: if pressing 'x' twice from the genre selection screen does not exit app, test won't exit and run indefinitely!): ");
        if (this.testForRratings()) {
            System.out.println("PASSED");
        } else {
            System.out.println("FAILED");
        }
        System.out.print(" Test 'r' loads the rating screen and if it selects a rating what happens(WARNING: if pressing 'x' twice from the genre selection screen does not exit app, test won't exit and run indefinitely!): ");
        if (this.testForRatingSlect()) {
            System.out.println("PASSED");
        } else {
            System.out.println("FAILED");
        }
    ...
    public boolean enterXToExit() {
        ...
        try {
            ...
            ((Frontend) frontend).run(new Backend(r));
    ...
    /**
     * This test runs the front end and redirects its output to a string. It then
     * passes in 'r' to go to the ratings select. The test succeeds if the front
     * end does contains all of the ratings by default. It fails if it doesnt
     * @return true if the test passed, false if it failed
     */
    public boolean testForRratings(){
        PrintStream standardOut = System.out;
        InputStream standardIn = System.in;
        try {
            // set the input stream to our input (with an g to test of the program lists genres)
            String input = "r" + System.lineSeparator() + "x" + System.lineSeparator() + "x"+ System.lineSeparator();
            InputStream inputStreamSimulator = new ByteArrayInputStream(input.getBytes());
            System.setIn(inputStreamSimulator);
            ByteArrayOutputStream outputStreamCaptor = new ByteArrayOutputStream();
            // set the output to the stream captor to read the output of the front end
    ...
    /**
     * tests to see if the rating select outputs the desired response
     * @return true if passes false if not
     */
    public boolean testForRatingSlect(){
        PrintStream standardOut = System.out;
        InputStream standardIn = System.in;
        try {
            // set the input stream to our input (with an g to test of the program lists genres)
            String input = "r" + System.lineSeparator() +"2"+System.lineSeparator()+ "x" + System.lineSeparator() + "x"+ System.lineSeparator();
            InputStream inputStreamSimulator = new ByteArrayInputStream(input.getBytes());
            System.setIn(inputStreamSimulator);
            ByteArrayOutputStream outputStreamCaptor = new ByteArrayOutputStream();
            // set the output to the stream captor to read the output of the front end
